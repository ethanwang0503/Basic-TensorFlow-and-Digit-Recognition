...
def my_perceptron(x):
    ...
    w = tf.get_variable(name ="weights", shape=[x],initializer=tf.constant_initializer(1.0), dtype=tf.float32)
...
def softmax_cross_entropy_with_logits(preds, Y):
	return -tf.reduce_sum(Y*tf.log(preds),reduction_indices=1)
...
def onelayer(X, Y, layersize=10):
    ...
    """
    w = tf.Variable(tf.truncated_normal([int(X.shape[1]), layersize], stddev=0.01 ))
    b = tf.Variable(tf.zeros([layersize]))
    logits = tf.matmul(X, w) + b
    preds = tf.nn.softmax(logits)
    batch_xentropy = softmax_cross_entropy_with_logits(preds, Y)
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    :return: The following variables should be returned in the following order.
        ...
        batch_loss: The average cross-entropy loss of the batch
    """
    w1 = tf.Variable(tf.truncated_normal([int(X.shape[1]), hiddensize], stddev=0.01))
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    """
    #conv1=tf.layers.conv2d(inputs=X,filters=convlayer_sizes[0],kernel_size=filter_shape,padding=padding,activation=tf.nn.relu,kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
